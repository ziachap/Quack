...
module Atoms =
    ...
    let NumberNode (identifier:Token) : AstNode =
        { Type = "NUMBER"; Value = identifier.Value; Children = [] }

    let IdentifierNode (identifier:Token) : AstNode =
        { Type = "LABEL"; Value = identifier.Value; Children = [] }

    let DeclareNode (child:AstNode) : AstNode =
        { Type = "DECLARE"; Value = child.Value; Children = [child] }

    let ArithmeticNode (left:AstNode, op:Token, right:AstNode) : AstNode =
        { Type = "ARITHMETIC_OPERATOR"; Value = op.Value; Children = [left; right] }
        
    let BooleanNode (left:AstNode, op:Token, right:AstNode) : AstNode =
        { Type = "BOOLEAN_OPERATOR"; Value = op.Value; Children = [left; right] }

    let FactorNode (exp:AstNode) : AstNode =
        { Type = "FACTOR"; Value = null; Children = [exp] }
        
    let PrintNode (exp:AstNode) : AstNode =
        { Type = "PRINT"; Value = null; Children = [exp] }

    let AssignNode (identifier:AstNode, exp:AstNode) : AstNode =
        { Type = "ASSIGN"; Value = identifier.Value; Children = [identifier; exp] }

    let StatementNode (statement:AstNode, next:AstNode) : AstNode =
        { Type = "STATEMENT"; Value = null; Children = [statement; next] }
        
    let FinalStatementNode (statement:AstNode) : AstNode =
        { Type = "STATEMENT"; Value = null; Children = [statement] }
    ...
    let IfNode (exp:AstNode, ifStatements:AstNode) : AstNode =
        { Type = "IF_ELSE"; Value = null; Children = [exp; ifStatements] }
        
    let IfElseNode (exp:AstNode, ifStatements:AstNode, elseStatements:AstNode) : AstNode =
        { Type = "IF_ELSE"; Value = null; Children = [exp; ifStatements;  elseStatements] }

    let WhileNode (exp:AstNode, whileStatements:AstNode) : AstNode =
        { Type = "WHILE"; Value = null; Children = [exp; whileStatements] }

    let FuncDefNode (identifier:AstNode, funcStatements:AstNode, funcParams:List<AstNode>) : AstNode =
        { Type = "FUNC_DEF"; Value = identifier.Value; Children = List.append [funcStatements] funcParams }
                
    let FuncInvokeNode (identifier:AstNode, funcParams:List<AstNode>) : AstNode =
        { Type = "FUNC_INVOKE"; Value = identifier.Value; Children = funcParams }